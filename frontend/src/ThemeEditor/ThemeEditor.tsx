...
const ColorEditor = React.memo((p: ColorEditorProps) => {
   const onColorChange = React.useCallback(
      (e: unknown, d: InputProps) =>
         p.onChange({ ...p.rule, [p.field]: d.value }),
      [p]
   );
   const toggleNone = React.useCallback(
      (e: unknown, d: CheckboxProps) =>
...
const RuleEditor = React.memo((p: RuleEditorProps) => {
   const onNameChange = React.useCallback(
      (e: unknown, d: InputProps) => p.onChange({ ...p.rule, name: d.value }),
      [p]
   );
...
const ThemeEditorConnected: React.FC<ThemeEditorProps> = p => {
   ...
   const onChange = React.useCallback(
      (e: unknown, data: DropdownProps) => setSelected(Number(data.value)),
      []
   );

   const onNameChange = React.useCallback(
      (e: unknown, data: InputProps) => {
         setModified(true);
