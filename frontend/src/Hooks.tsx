...
/**
 * A hook that only updates a variable after a short delay.
 * The idea is that a callback has to be run on every change (typically on a
 * form element), because we need the new value to update the element's value.
 * However, the rest of the code should be watching a debounced value so that
 * it doesn't need to run as often.
 */

export const useDebounce = <T extends never>(val: T, delayms?: number): T => {
...
export function createSelector<V>(resolver: (...p: unknown[]) => V) {
   const selector = createMemoSelector(resolver);
   return (...dependencies: unknown[]) => selector(dependencies);
}
