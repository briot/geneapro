...
export default class Quilts extends React.PureComponent<QuiltsProps, QuiltsState> {
   ...
   addToSelection(p: QuiltsPersonLayout) {
      ...
      for (let s = 0; s < this.state.selected.length; s++) {
         if (this.state.selected[s].base === p) {
            this.setState(old => ({
               selected: old.selected.slice(0, s).concat(
                  old.selected.slice(s + 1)),
            }));
            return;
         }
      }
   ...
   render() {
       ...
       */
      const addParents = (uniqueId: string,
         ...
         const X = Math.max(
            ...
            childFam ?
               ...
               Math.max.apply(null, p.parentFamilies.map(fam => fam.left + LINE_SPACING)));
         selectionLines.push(
            <rect
               key={uniqueId}
         ...
         p.childFamilies.forEach((fam, famIndex) => {
            ...
            if (father) {
               addParents(
                  ...
                  father, color, color, fam /* childFam */);
            }
            if (mother) {
               addParents(
                  `${uniqueId}-m${mother.person.id}`,
                  mother, color, color, fam /* childFam */);
            }
         });
      };

      const addChildren = (uniqueId: string,
                           p: QuiltsPersonLayout,
                           color: string
                          ) => {
               ...
               if (child) {
                  ...
                  selectionLines.push(
                     <rect
                        ...
                        y={child.topY}
                        height={child.bottomY - child.topY - 2 * SELECTION_MARGIN}
                        fill={color}
                     />
                  );
      ...
      this.state.selected.forEach((s, selectedIndex) => {
         ...
         addParents(
            ...
            s.base, c.toString(), s.color.toString());
         addChildren(
            `c${selectedIndex}-${s.base.person.id}`,
            s.base, s.color.toString());
      });
