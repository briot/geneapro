...
function circle(
   ...
   const style = ColorTheme.forPerson(
      settings.colors, settings.generations, node.data.p, node.data);
   ...
   return (
      ...
      >
         <g
            className="node"
            transform={`rotate(${(node.x * 180) / Math.PI - 90})translate(${
               node.y
            })`}
         >
            <circle
...
export default function Radial(props: RadialProps) {
   const diameter =
      2 * Math.abs(props.settings.generations) * props.settings.spacing;
   ...
   function buildTree(
      ...
      const relatives =
         (props.settings.generations > 0 ? p.parents : p.children) || [];
   ...
   // but multiple links
   let coords: {
      [id: number]: d3Hierarchy.HierarchyPointNode<RadialLayout>;
   } = {};
   ...
   const lines = tree
      .links()
      .map((link, index) => (
         <path className="link" key={index} d={radialLink(link) || ""} />
      ));
   ...
   return (
      <ScalableSVG
         className={"Radial " + (props.settings.showText ? "visibleNames" : "")}
      >
         <defs>{defs}</defs>
