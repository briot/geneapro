import * as React from 'react';
import { PedigreeSettings, ColorScheme, LinkStyle, isTopDown } from '../Store/Pedigree';
import { PersonLayout } from '../Pedigree/types';

interface PedigreeLinkProps {
   from: PersonLayout;
   to: PersonLayout;
   style: PedigreeSettings;
}

export default function PedigreeLink(props: PedigreeLinkProps) {
   const topDown = isTopDown(props.style);

   const linkX = (p: PersonLayout, isFrom: boolean) => {
      return topDown ?
         p.x + p.w / 2 :
         isFrom ? p.x + p.w : p.x;
   };

   const linkY = (p: PersonLayout, isFrom: boolean) => {
      return topDown ?
         (isFrom ? p.y + p.h : p.y) :
         p.y + (props.style.colors === ColorScheme.NO_BOX ?
            p.fs + 1 : p.h / 2);
   };

   const source = [linkX(props.from, true /* isFrom */),
                   linkY(props.from, true /* isFrom */)];
   const target = [linkX(props.to, false /* isFrom */),
                   linkY(props.to, false /* isFrom */)];

   let d: string;

   switch (props.style.links) {
      case LinkStyle.STRAIGHT:
         d = 'M' + source + 'L' + target;
         break;

      case LinkStyle.ORTHOGONAL:
         d = topDown ?
            ('M' + source + 'V' + (target[1] - 5) +
              'H' + target[0] + 'V' + target[1]) :
            ('M' + source + 'H' + (target[0] - 5) +
              'V' + target[1] + 'H' + target[0]);
         break;

      default:  // CURVE
         d = topDown ?
            ('M' + source +
               'C' + source[0] + ',' + (source[1] + target[1]) / 2 +
               ' ' + target[0] + ',' + (source[1] + target[1]) / 2 +
               ' ' + target) :
            ('M' + source +
               'C' + (source[0] + target[0]) / 2 + ',' + source[1] +
               ' ' + (source[0] + target[0]) / 2 + ',' + target[1] +
               ' ' + target);
   }

   if (props.style.colors === ColorScheme.NO_BOX) {
      d += topDown ?
         'M' + source + 'v-5' :
         'M' + source + 'h' + (-props.from.w);
   }

   return (
      <path
         className="link"
         d={d}
      />
   );
}
