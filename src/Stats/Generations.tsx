...
const StatsGeneration = React.memo((p: StatsGenerationProps) => {
   ...
   React.useEffect(() => {
      ...
      const x: d3Axis.AxisScale<number> = d3Scale
         .scaleLinear()
         .domain([
            Math.min.apply(null, p.ranges.map(d => d[1])),
            Math.max.apply(null, p.ranges.map(d => d[2]))
         ])
         .range([margin, width]);
      const y = d3Scale
         .scaleLinear()
         .domain([p.ranges[0][0], p.ranges[p.ranges.length - 1][0]])
      ...
      // The bars
      const r = svg
         .selectAll<SVGRectElement, JSONGenerationRange>("rect")
         .data(p.ranges);
      r.enter()
         .append("rect")
   ...
   return (
      <Card fluid={true} className="Stats">
         <Card.Content>
            <Card.Header>
               Generations timespan (oldest birth - latest death), in {name}
               &apos;s tree
            </Card.Header>
            <Card.Description>
