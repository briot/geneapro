       ...
       contextMenu : ContextMenu) =>
   {
      return {
            ...
            element : angular.IAugmentedJQuery)
         {
            ...
            scope.$watch(
               () => [scope.decujus, $rootScope.settings],
               () => {
            ...
            function render(data ?: PedigreeService) {
               ...
               node.append("circle").attr("r", circleSize)
                  .each(function(d) { return d3.select(this).attr(<any>styles.style(d))})
                  .attr('title', function(d) { return data.displayName(d.p)});
               ...
               if (set.showText) {
                  node.append("text")
                   .attr({
                      dy: '.31em',
                      'text-anchor': d => d.x < 180 ? "start" : "end",
                      'transform': d => d.x < 180 ? "translate(8)" : "rotate(180)translate(-8)"})
