...
factory('CitationTemplates', function($http, $q) {
   ...
   class CitationTemplate {
      ...
      constructor(data) {
         this.full = data.full;
         this.biblio = data.biblio;
         this.abbrev = data.short;
         this.fields = [];
   ...
   class CitationTemplates {
      ...
      all_models() {
         ...
         if (this.models.length != 0) {
            ...
            $http.get('/data/citationModels').then(resp => {
               this.models = [{id: '',
       ...
       */
      get_templates(model_id) {
         const d = $q.defer();
         if (this.details[model_id]) {
            d.resolve(this.details[model_id]);
         } else {
            $http.get('/data/citationModel/' + model_id).then(resp => {
               d.resolve(
                  this.details[model_id] = new CitationTemplate(resp.data));
            }, () => {
               d.reject();
            });
         }
...
directive('gpSourceCitation', function(CitationTemplates, $http, $state) {
   return {
      ...
      controller: function($scope) {
         ...
         CitationTemplates.all_models().then(models => {
            $scope.source_types = models;
         });

         $scope.$watch('source.medium', val => {
            CitationTemplates.get_templates(val).then(template => {
         ...
         function computeCitation() {
            const c = (
               (s.medium && $scope.citation)
               ? $scope.citation.cite($scope.cache)
               : saved_citation);
            if (c) {
               s.title = c.full;
               s.biblio = c.biblio;
         ...
         $scope.saveParts = function() {
           ...
           const data = angular.copy($scope.source);
           angular.forEach($scope.citation.fields, name => {
              if ($scope.cache[name]) {
...
controller('sourceCtrl', function($scope, $http, $state, $stateParams) {
   ...
   // ??? Should use a service instead
   $http.get('/data/sources/' + id).then(resp => {
      parse_SourceView_response($scope, resp);
   });
