...
app.factory('gpd3', function() {
   ...
   class gpd3 {
       ...
       */
      static setViewBox(element, layout, preventZoomIn, margin=10) {
      ...
      static linksDraw(style) {
         let link_y = function(person) {
       ...
       */
      static gradientForEachGeneration(group, maxGen) {
       ...
       * @return {function}       Given a person, returns its fill style.
       */
      static gradientForEachAngle(group, nodes, color0, color1, isRadial) {
         ...
         if (isRadial) {
            g = group.append("defs").selectAll('radialGradient')
       ...
       */
      static getStyles(group, nodes, settings, data, preserve, isRadial) {
         ...
         switch(settings.colorScheme) {
            case gpd3.colorScheme.GENERATION:
               if (settings.appearance == gpd3.appearance.GRADIENT) {
                  ...
                  result.fillStyle = function(p) {
                     const gen = Math.abs(p.generation);
                     return gpd3.hsb(180 + 360 * (gen - 1) / 12, 0.4, 1.0);
                  };
               }
               break;
            ...
            case gpd3.colorScheme.PEDIGREE:
               ...
               if (settings.appearance == gpd3.appearance.GRADIENT) {
                   result.fillStyle = gpd3.gradientForEachAngle(
                      ...
                      result.fillStyle,
                      d => {
                         const gen = Math.abs(d.generation);
            ...
            case gpd3.colorScheme.RULES:
                result.fillStyle = function(p) {
                   return data.getStyle(p)['fill'];
                };
                result.strokeStyle = function(p) {
                   return data.getStyle(p)['stroke'];
                };
                result.textStyle = function(p) {
                   return data.getStyle(p)['color'];
                };
            ...
            case gpd3.colorScheme.QUARTILE:
               ...
               angular.forEach(nodes, function(d) {
                  const m = Math.pow(2, d.generation);
               ...
               if (settings.appearance == gpd3.appearance.GRADIENT) {
                  const fillStyle = gpd3.gradientForEachAngle(
                     group,
                     nodes,
                     result.fillStyle,
