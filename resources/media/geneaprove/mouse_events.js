...
;(function($) {  // Self-executing function, converts jQuery to "$"
  ...
  $.fn.ctrl_alt_shiftclick = function(d, fn) {
     return this[(fn || d) ? 'bind' :
       'trigger']('ctrl-alt-shift-click', d, fn)};
  ...
  var  _fired = 'fired.clickAH';
  var  _weight = 'weight.AH';
  var  hold_duration = 200;  //  milliseconds
  var  _onIOS = (/iphone|ipad|ipod/i).test(navigator.userAgent);
          ...
          if (this.addEventListener) {
             ...
             for (var i = types.length - 1; i >= 0; i--) {
                this.addEventListener(
                      types[i], $.event.special.mousewheel.handler, false);
             }
          } else {
  ...
  function _cancel(e) {
     var $t = $(this);
     $t.data(_timer, clearTimeout(/** @type{undefined|number} */($t.data(_timer))) || null)
  ...
  var dt = 0.01; // interval in seconds between two refresh when throwing
  var Threshold = 0.01;  // 1% of initial velocity stops the element
  var Time = 0.7;        // stops moving at this many seconds
  var storePast = 500;   // save position data for the last ... milliseconds
  var _throwtimer = 'timerTHROW';
  var _dragData = 'dataTHROW';
